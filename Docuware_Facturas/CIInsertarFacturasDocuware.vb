...
Public Class CIInsertarFacturasDocuware
    Inherits Solmicro.Expertis.Engine.UI.CIMnto

    Dim basedatos As String
    Dim insertada As String
    Dim transferida As String
    ...
    Private Sub CIInsertarFacturasDocuware_QueryExecuting(ByVal sender As System.Object, ByRef e As Solmicro.Expertis.Engine.UI.QueryExecutingEventArgs) Handles MyBase.QueryExecuting
        With e.Filter
            If txtSuFactura.Text <> "" Then .Add("SuFactura", FilterOperator.Equal, txtSuFactura.Text, FilterType.String)
            If advProveedor.Text <> "" Then .Add("IDProveedor", FilterOperator.Equal, advProveedor.Value, FilterType.String)
            If advNObra.Text <> "" Then .Add("NObra", FilterOperator.Equal, advNObra.Value, FilterType.String)
            If cbBasededatos.Value <> "" Then
                Select Case cbBasededatos.Value.ToString
                    Case "TECOZAM"
                        basedatos = "001"
                    Case "FERRALLAS"
                        basedatos = "002"
                    ...
                    Case "TECOZAM_DEMO"
                        basedatos = "004"
                End Select
                .Add("BaseDeDatos", FilterOperator.Equal, basedatos, FilterType.String)
            End If
            .Add("SuFechaFactura", FilterOperator.GreaterThanOrEqual, cbFIni.Value, FilterType.DateTime)
            .Add("SuFechaFactura", FilterOperator.LessThanOrEqual, cbFFin.Value, FilterType.DateTime)
            'Insertada
            If cbInsertada.Value <> "" Then
                Select Case cbInsertada.Value.ToString
                    Case "SI"
                        insertada = 1
                        .Add("Insertada", FilterOperator.Equal, "1")
                    Case "NO"
                        insertada = 0
                        .Add("Insertada", FilterOperator.Equal, "0")
                    Case "TODAS"
                End Select
            End If

            'Transferida
            If cbTransferida.Value <> "" Then
                Select Case cbTransferida.Value.ToString
                    Case "SI"
                        transferida = 1
                        .Add("Transferida", FilterOperator.Equal, transferida)
                    Case "NO"
                        transferida = 0
                        .Add("Transferida", FilterOperator.Equal, transferida)
                    Case "TODAS"
    ...
    Private Sub CIInsertarFacturasDocuware_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        CargaCombos()
    ...
    Public Sub CargaBaseDatos()
        Dim dtBaseDeDatos As New DataTable
        dtBaseDeDatos.Columns.Add("BASE DE DATOS")

        dtBaseDeDatos.Rows.Add("TECOZAM")
        dtBaseDeDatos.Rows.Add("FERRALLAS")
        dtBaseDeDatos.Rows.Add("SECOZAM")
        dtBaseDeDatos.Rows.Add("TECOZAM_DEMO")

        dtBaseDeDatos.AcceptChanges()
        cbBasededatos.DataSource = dtBaseDeDatos
    End Sub

    Public Sub CargaInsertada()
        Dim dt As New DataTable
        dt.Columns.Add("INSERTADA")
        ...
        dt.AcceptChanges()
        cbInsertada.DataSource = dt
    End Sub

    Public Sub CargaTransferida()
        Dim dt As New DataTable
        dt.Columns.Add("TRANSFERIDA")
...
#Region "Insertar Facturas Tecozam50R2 Demo"
    ...
    Public Sub InsertFacturasTecozamDemo()
        ...
        Dim selectedRows As DataTable = Grid.DataSource
        
        Dim registrosFiltrados() As DataRow
        registrosFiltrados = selectedRows.Select("Expertis.CheckValue = True")

        Dim dtFiltrado As New DataTable
        If registrosFiltrados.Any() Then
            dtFiltrado = registrosFiltrados.CopyToDataTable()
        End If

        'Dim tabla As DataTable = New BE.DataEngine().Filter(Grid.DataSource, filtro)
        'Compruebo que todos los articulos de la tabla existen en bbdd
        Dim factura As String
        factura = comprueboExistenciaArticulo(dtFiltrado)
        If factura.Length = 0 Then
            'Llamo al método que va a realizar la inserción
            insertarFacturas(dtFiltrado)
        Else
    ...
    Private Sub Grid_RecordChecked(ByVal sender As System.Object, ByVal e As Solmicro.Expertis.Engine.UI.CheckedEventArgs) Handles Grid.RecordChecked
        ...
        For Each dr1 As DataRow In dt.Rows
            For Each dr As DataRow In dtGrid.Rows
                If dr("IDFacturaDocuware") = dr1("IDFacturaDocuware") Then
                    dr("Expertis.CheckValue") = True
                End If
            Next
        Next
